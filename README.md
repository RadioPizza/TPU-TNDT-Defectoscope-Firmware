...
- Джойстик K-Silver JH16 с датчиками Холла
  ...
  - Пины подключения: `JOY_X_PIN` - осьX (АЦП1), `JOY_Y_PIN` - осьY (АЦП1), `JOY_BTN_PIN` - кнопка (вход с подтяжкой)
...
- Кнопка с подсветкой для индикации на 3,3 В. Внутри аппаратного антидребезга вероятно нет. Снаружи тоже не предусматривается. Лучше сделать цифровой антидребезг 50-75 мс.
  - Пины подключения: `START_BTN_GND` - земля (цифровая), `START_BTN_PIN` - кнопка (вход с подтяжкой), `START_LED_PIN` - светодиод (выход ШИМ)
...
- LED-подсветка рабочей области: два белых светодиодов мощность 200 мВт. Номинальный ток 20 мА при напряжении 3 В. Подключаются напрямую к плате.
  - Пины подключения:
    Светодиод №1: `WORK_LIGHT1_GND` - земля (цифровая), `WORK_LIGHT1_PIN` - сигнальный;
    Светодиод №2: `WORK_LIGHT2_GND` - земля (цифровая), `WORK_LIGHT2_PIN` - сигнальный;

- Переключатель разрешения активации нагревательных ламп
  - Пины подключения: `HEATER_ENABLE_GND` - земля (цифровая), `HEATER_ENABLE_PIN` - сигнальный.

> Наличие аппаратных элементов и их пины подключения указываются в `src/config.h
